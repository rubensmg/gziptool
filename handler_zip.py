...
from os.path import relpath, join as path_join, exists as path_exists
...
def compress():
    ...
    with ZipFile(f'{filename}.zip', 'w') as zip_file:
        def add_file(directory, f):
            ...
            zip_file.write(full_path, compress_type=ZIP_DEFLATED)

        [[add_file(dirpath, f) for f in filenames] for dirpath, _, filenames in walk(path)]
        
    print(f'{COLOR_GREEN}File {filename}.zip created{COLOR_WHITE}')
...
def decompress():
    ...
    with ZipFile(f'{filename}.zip') as zip_file: 
        zip_file.extractall(path=path)

    print(f'{COLOR_GREEN}File {filename}.zip decompressed in {path}{COLOR_WHITE}')
